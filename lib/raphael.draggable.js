...
(function() {  
    ...
    function _startDragger() {
      document.onmousemove = function(event) {
        if (paper.draggable.current()) {
          ...
          _triggerHandler(paper.draggable.current(), "drag");
        }
      };
    }
  };
   ...
   **/
  function _triggerHandler(element, event) {
    var handlers = element.draggable.handlers[event];
    if (handlers) {
      for(var i = 0, len = handlers.length; i < len; i++) {
        handlers[i].apply(element);
      }
    }

    var children = element.items;
    if (children) {
      _triggerForEach(children, event);
    }
  }
   ...
   * @param{String} event The name of the event to trigger
   **/
  function _triggerForEach(elements, event) {
    for(var i = 0, len = elements.length; i < len; i++) {
      _triggerHandler(elements[i], event);
    }
  }
})();
